...
def savenpy_tianchi(id,filelist,prep_folder,data_path,use_existing=True):      
    ...
    if use_existing:
        if os.path.exists(os.path.join(prep_folder,name+'_label.npy')) and os.path.exists(os.path.join(prep_folder,name+'_clean.npy')):
            ...
            return
    try:
        im, m1, m2, spacing, origin = step1_python_tianchi(os.path.join(data_path,name))
        Mask = m1+m2
...
def full_prep_tianchitest(data_path, prep_folder, n_worker=None, use_existing=True):
    ''' 
        'datapath':'/mnt/media/wentao/tianchi/', # use val_subset02, val_subset03, val_subset04
