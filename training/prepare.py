...
# from step1 import step1_python_tianchi
# from step1_python import step1_python_tianchi
import warnings
...
def full_prep(train=True, val=True, test=True):
    ...
    if not os.path.exists(finished_flag):
        ...
        if train:
            ...
            for i in xrange(len(trainfilelist)):
                savenpy(i, trainalllabel, trainfilelist, train_data_path, train_prep_folder) # debug only
            # pool = Pool(10)
        ...
        if val:
            ...
            for i in xrange(len(valfilelist)):
                savenpy(i, valalllabel, valfilelist, val_data_path, val_prep_folder) # debug only
        ...
        if test:
            ...
            for i in xrange(len(testfilelist)):
                savenpy(i, testalllabel, testfilelist, test_data_path, test_prep_folder) # debug only
...
#             testfiles.append(f[:-4])
#     import csv 
#     testf = open(config['test_annos_path']+'seriesuids.csv', 'w')
#     testfcsv = csv.writer(testf)
...
#         if f.endswith('mhd'):
#             valfiles.append(f[:-4])
#     valf = open(config['val_annos_path']+'seriesuids.csv', 'w')
#     valfcsv = csv.writer(valf)
...
if __name__=='__main__':
    # if not os.path.exists(config['test_annos_path']): # for once
    #     os.mkdir(config['test_annos_path'])
    #     splitvaltestcsv()

    full_prep(train=True, val=True, test=True)
    # prepare_luna()
    # preprocess_luna()
